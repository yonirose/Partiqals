...
class DigikeySpider(scrapy.Spider):
    ...
    # total_pages = 0
    page_count = 0
    scan_batch = cfg.BATCH_SIZE # 0 to 1
    batch_count = 0
        ...
        for idx in range(1, table_len+1):
            ...
            db.manufacdb.update_one(
                    ...
                    {'$setOnInsert': {'manufac': part['manufac'],
                                      'manufac_ngram3': lp.Parser.get_ngrams(
                                                        part['manufac'])}},
                     upsert=True
                )
    ...
    def save_pdf(self, response):
        upart = response.meta['item']['part_num'] + '__' \
                + response.meta['item']['manufac']
        db.metadb.update_one({'upart': upart},
                             {'$setOnInsert': {'upart': upart},
        ...
        if cfg.KEEP_GRIDFS_PDF:
            ...
            self.logger.info('### Saved PDF link %s ###' % upart)
